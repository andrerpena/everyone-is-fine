...
export const colonistInspectorSchema = nu
  .object({
    ...
    thoughts: nu.string().withMetadata({
      label: "Thoughts",
      renderer: "readonly",
      editable: false,
    }),
    relationships: nu.string().withMetadata({
      label: "Relationships",
      renderer: "readonly",
      editable: false,
    }),
  ...
  .withFormLayouts({
    default: {
      ...
      groups: [
        ...
        {
          label: "Relationships",
          fields: [{ name: "relationships", fieldWidth: 12 }],
        },
      ],
    },
  });
